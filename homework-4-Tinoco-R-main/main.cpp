...
int main() {
  ...
  */
  // Modified UV coordinates to flip texture upside down
  static const GLfloat uv[] = {
      0.000059f, 0.000004f, 0.000103f, 0.336048f,
      ...
      0.668104f, 0.000013f, 0.335973f, 0.335903f,
      0.667979f, 0.335851f, 0.335973f, 0.335903f,
      0.668104f, 0.000013f, 0.336098f, 0.000071f,
  };
  ...
  glGenBuffers(1, &VBO);
  // Generate buffer object for UV Buffer
  glGenBuffers(1, &UVBO);
  ...
  // Setting up UVBO with uv float array
  // First, bind UVBO to GL_ARRAY_BUFFER and then populate data within it
  glBindBuffer(GL_ARRAY_BUFFER, UVBO);
  glBufferData(GL_ARRAY_BUFFER, sizeof(uv), uv, GL_STATIC_DRAW);
  
  // UV Coordinate attribute
  // Next, setup the vertex attribute pointer to point to the UV coordinates at location 1
  glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), (GLvoid *)0);
  glEnableVertexAttribArray(1);

  // Unbind VAO and rebind to containerVAO
  glBindVertexArray(0);
  glBindVertexArray(containerVAO);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Since no camera object used this time, set value to be arbitrarily 45
    ...
    // This is already being done within the loadDDS function, so will simply call it
    textureID = loadDDS("texture.dds");
